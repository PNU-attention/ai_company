...
5. Rube 연결 상태 모니터링 (Rube 설정 있는 경우 항상 실행)
   ...
   │                                                              │
   │  [기존 인벤토리 있는 경우]                                    │
   │  tool_inventory.json의 mcp_rube 목록을 읽어                  │
   │  RUBE_MANAGE_CONNECTIONS 실시간 호출 → 현재 상태 확인         │
   │                                                              │
   │  변경 감지 시:                                                │
   │   ✅ 새로 연결된 앱 → 인벤토리 자동 갱신 + CEO 알림          │
   │   ⚠️ 연결이 끊긴 앱 → 인벤토리 자동 갱신 + CEO 경고         │
   │   → tool_inventory.json 자동 패치                           │
   │                                                              │
   │  [인벤토리 없는 경우]                                        │
   │  → 건너뜀 (Step 3에서 Tool Agent가 신규 생성)               │
   └──────────────────────────────────────────────────────────────┘

   tool-agent를 "Phase 0: 연결 상태 갱신" 모드로 호출하여 수행합니다.
